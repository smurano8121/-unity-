...
public class WebcamWeather : MonoBehaviour
{
	...
	IEnumerator Start()
	{
		...
		enabled = false;
		yield return new WaitForSeconds(.1f); //天気情報を取得するまで処理を止める
        enabled = true;
		//動画と音が再生できるように用意
	...
	void Update()
	{
        ...
        if (nownum < 130) { nowtime = "morning"; }
		else if (sunrizenum-8 <= nownum && nownum < sunrizenum) {	nowtime = "morning"; }
		...
		nowinfo = "file://C:/疑似窓/VirtualWindowAssets/動画/"+nowseason +"_" + nowweather + "_" + nowtime + ".mp4";
        Debug.Log(nownum);
		Debug.Log(nowinfo);
	...
	//天気情報をOpenWeatherのAPIから30分おきに取得
	IEnumerator Weather()
	{
		Debug.Log("refresh");
